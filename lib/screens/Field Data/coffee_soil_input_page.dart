...
class _CoffeeSoilInputPageState extends State<CoffeeSoilInputPage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _plotIds = _farmingScenario == 'multiple' ? ['Plot 1'] : [_farmingScenario == 'intercrop' ? 'Intercrop' : 'SingleCrop'];
  ...
  Future<void> _checkPlotDataStatus() async {
    ...
    for (var plotId in _plotIds) {
      final snapshot = await FirebaseFirestore.instance
          ...
          .get();
      if (mounted) setState(() => _plotHasData[plotId] = snapshot.docs.isNotEmpty);
    }
  ...
  Future<void> _showOnboardingDialog() async {
    ...
    final result = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        title: const Text('Redefine Farming Structure', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF4A2C2A))),
            ...
            child: Column(
              ...
              children: [
                if (_farmingScenario == 'multiple') ...[
                  TextFormField(
                    decoration: const InputDecoration(labelText: 'Number of Plots', border: OutlineInputBorder()),
                    keyboardType: TextInputType.number,
                    onChanged: (value) => plotCount = int.tryParse(value),
                  ),
                  const SizedBox(height: 16),
                  TextFormField(
                    decoration: const InputDecoration(labelText: 'Plot Label Prefix', border: OutlineInputBorder()),
        ...
        actions: [
          TextButton(
            onPressed: () {
              if (_farmingScenario != 'multiple' || (plotCount != null && plotCount! > 0)) {
                if (mounted) {
                  setState(() {
                    if (_farmingScenario == 'multiple') {
                      _plotIds = List.generate(plotCount!, (i) => '${plotLabelPrefix ?? 'Plot'} ${i + 1}');
                      _tabController.dispose();
                      _tabController = TabController(length: _plotIds.length, vsync: this);
                    }
                  });
    ...
    if (result != true && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Structure not changed')));
    }
  }

  void _showFieldHistory() {
    Navigator.push(context, MaterialPageRoute(builder: (context) => CoffeeSoilSummaryPage(userId: FirebaseAuth.instance.currentUser!.uid)));
  }
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: const Text('Soil Data Input', style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
        ),

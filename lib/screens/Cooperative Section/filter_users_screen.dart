...
class _FilterUsersScreenState extends State<FilterUsersScreen> {
  ...
  Future<void> _updateSuggestions() async {
    ...
    try {
      final userSnapshot =
          await FirebaseFirestore.instance.collection('${formattedCoopName}_users').get();
      final marketManagerSnapshot =
          await FirebaseFirestore.instance.collection('${formattedCoopName}_marketmanagers').get();
  ...
  Future<void> _downloadExcel(List<Map<String, dynamic>> filteredUsers) async {
    try {
      ...
      // Fallback to hardcoded path for Android
              ...
              builder: (context, userSnapshot) {
                ...
                return StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (context, marketManagerSnapshot) {
                    ...
                    List<Map<String, dynamic>> users = userSnapshot.data!.docs.map((doc) {
                    ...
                    if (marketManagerSnapshot.hasData) {
                      for (var doc in marketManagerSnapshot.data!.docs) {
                        final data = doc.data() as Map<String, dynamic>;
                        int index = users.indexWhere((u) => u['uid'] == doc.id);
                        if (index >= 0) {
                          users[index]['role'] = 'Market Manager';
                          users[index].addAll({
                            'fullName': data['fullName'] ?? users[index]['fullName'],
                            'email': data['email'] ?? users[index]['email'],
                            'county': data['county'] ?? users[index]['county'],
                            'constituency': data['constituency'] ?? users[index]['constituency'],
                            'ward': data['ward'] ?? users[index]['ward'],
                            'phoneNumber': data['phoneNumber'] ?? users[index]['phoneNumber'],
                          });
                        }
                      }
                    }
